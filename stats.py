...
def compute_stats(grouped_messages):
    ...
    with ProcessPoolExecutor(max_workers=4) as executor:
        users = dict(list(executor.map(_user_stats, grouped_messages.items())))

    return {
        'group': {
            name: fn([m for ms in grouped_messages.values() for m in ms])
            for name, fn in _group_stat_functions.items()
        },
        'users': users
    }
...
def _user_stats(args):
    user, messages = args
    return (user, {name: fn(messages) for name, fn in _user_stat_functions.items()})
...
def _compute_sentiment(messages):
    ...
    return {
        ...
        'positive': len([v for v, _ in sentiments if v > 0]),
        'most_positive': max(sentiments),
        'negative': len([v for v, _ in sentiments if v < 0]),
